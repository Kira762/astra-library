#!/bin/sh
# Runtime test for the slider knob's travel: the capsule's centre stays half a
# knob inside each track end (so it never overlaps the card edge at max or
# min), and the fill ends at the knob's centre, resting and held alike.
#
...
OUT="$TMPDIR_LOCAL/astra_slider_travel_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/slider_travel_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "SLIDER TRAVEL TEST PASSED"
	exit 0
else
	echo "SLIDER TRAVEL TEST FAILED (see above)" >&2
	exit 1
fi
