#!/bin/sh
# Runtime test for in-card element descriptions: the description label is a
# child of the element main, the card grows by the measured line height,
# controls keep centring in the base region, and nothing renders below.
#
...
OUT="$TMPDIR_LOCAL/astra_inline_description_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/inline_description_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "INLINE DESCRIPTION TEST PASSED"
	exit 0
else
	echo "INLINE DESCRIPTION TEST FAILED (see above)" >&2
	exit 1
fi
