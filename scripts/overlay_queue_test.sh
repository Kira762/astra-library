#!/bin/sh
# Runtime test for the overlay entrance queue (`components/overlayQueue.luau`)
# and the staged window entrance.
#
...
OUT="$TMPDIR_LOCAL/astra_overlay_queue_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/overlay_queue_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "OVERLAY QUEUE TEST PASSED"
	exit 0
else
	echo "OVERLAY QUEUE TEST FAILED (see above)" >&2
	exit 1
fi
