...
| `CreateAboutCard({ name, subtitle, icon, rows = { { icon, label, value } }, description, action = { icon, name, subtitle, callback } })` | one card: brand header, **1–3** data rows packed to the card's width (extra rows wrap, never truncate), description paragraph and an optional tappable action band (a fourth row errors) | `:SetTitle(name)`, `:SetSubtitle(text?)`, `:SetIcon(icon)`, `:SetRow(index, row)`, `:SetDescription(text?)` |
