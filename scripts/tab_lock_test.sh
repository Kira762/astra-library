#!/bin/sh
# Runtime test for locked tabs (`CreateTab({ locked = true })` + `tab:SetLocked`).
#
...
OUT="$TMPDIR_LOCAL/astra_tab_lock_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/tab_lock_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "TAB LOCK TEST PASSED"
	exit 0
else
	echo "TAB LOCK TEST FAILED (see above)" >&2
	exit 1
fi
