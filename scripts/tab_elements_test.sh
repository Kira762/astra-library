#!/bin/sh
# Runtime test for components/window.luau + elements/tab.luau: a tab that was
# never opened is not walked element by element on every show/hide - it is
# marked and shown when the player opens it (or when the search renders it),
# while every tab's visible state stays exactly what it was.
#
...
OUT="$TMPDIR_LOCAL/astra_tab_elements_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/tab_elements_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "TAB ELEMENTS TEST PASSED"
	exit 0
else
	echo "TAB ELEMENTS TEST FAILED (see above)" >&2
	exit 1
fi
