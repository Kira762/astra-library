...
- `stepper.luau` — the `[−] [value] [+]` value counter (`__type = "Stepper"`): two icon-style tap buttons (resolved `minus`/`plus` glyphs on the `NeutralButton` surface with hover/press feedback) flanking the focal value field — the input family's field surface wearing the accent stroke at rest and the larger `valueTextSize` so the number reads first. `step` is one tap and the display's precision (re-rounding on every write kills ±drift), `min` (default `0`) is the floor both taps respect, `max` is optional (no ceiling by default), and typed commits round, clamp and restore unparseable text. Callbacks fire only on real changes; `Set`/`Get`/`Increment`/`Decrement`/`CancelEditing` round out the API. Joins the slider's compact-row slot and lock tier (3).
...
| `stepper_value_test.sh` | Stepper values: the `0` default, per-click `±step` with the display following, the `0` floor (a `(−)` tap there is a no-op with no callback), the optional `max` ceiling, typed commits clamping into range and unparseable text restoring the old value, and the callback firing only on real changes. |
