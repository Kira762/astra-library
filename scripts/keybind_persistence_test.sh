...
OUT="$TMPDIR_LOCAL/astra_keybind_persistence_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/keybind_persistence_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "KEYBIND PERSISTENCE TEST PASSED"
	exit 0
else
	echo "KEYBIND PERSISTENCE TEST FAILED (see above)" >&2
	exit 1
fi
