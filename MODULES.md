...
- `stepper.luau` — the `[−] [value] [+]` value counter (`__type = "Stepper"`): two icon-style tap buttons (resolved `minus`/`plus` glyphs on the `NeutralButton` surface with hover/press feedback) flanking the value field inside one pill capsule — the whole row wears the same `SurfaceStroke` hairline (the field rests at the input family's 0.85 and tightens to 0.4 while focused), and the number is the focal point by size alone, on the larger `valueTextSize`. A passed `description` is ignored like the rest of the control family: the card stays a single 41px row with the title block and the cluster on one centre line. `step` is one tap and the display's precision (re-rounding on every write kills ±drift), `min` (default `0`) is the floor both taps respect, `max` is optional (no ceiling by default), and typed commits round, clamp and restore unparseable text. Callbacks fire only on real changes; `Set`/`Get`/`Increment`/`Decrement`/`CancelEditing` round out the API. Joins the slider's compact-row slot and lock tier (3).
