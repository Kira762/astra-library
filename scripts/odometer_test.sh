#!/bin/sh
# Runtime test for utilities/odometer.luau: rows are materialised per
# transition instead of all 20 per digit, while the readout (the row the strip
# rests on) still shows the value's digit and still lands correctly after a
# plain change, a wrap and a roll down.
#
...
OUT="$TMPDIR_LOCAL/astra_odometer_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/odometer_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "ODOMETER TEST PASSED"
	exit 0
else
	echo "ODOMETER TEST FAILED (see above)" >&2
	exit 1
fi
