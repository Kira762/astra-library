#!/bin/sh
# Runtime test for icons/init.luau: a bare icon name searches every pack in a
# deterministic priority order (lazily), `pack:name` addresses one pack exactly,
# lookups stay case-sensitive, and custom assets resolve with the import done
# once per path - misses included, so a page build stops paying for files that
# are not there.
#
...
OUT="$TMPDIR_LOCAL/astra_icons_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/icons_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "ICONS TEST PASSED"
	exit 0
else
	echo "ICONS TEST FAILED (see above)" >&2
	exit 1
fi
