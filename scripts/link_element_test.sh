...
OUT="$TMPDIR_LOCAL/astra_link_element_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/link_element_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "LINK ELEMENT TEST PASSED"
	exit 0
else
	echo "LINK ELEMENT TEST FAILED (see above)" >&2
	exit 1
fi
