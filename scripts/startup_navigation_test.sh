#!/bin/sh
# Runtime test: a tab (or Settings) the user opens while the host is still
# building the window stays open when the host's closing `tab:Select()` runs.
#
...
OUT="$TMPDIR_LOCAL/astra_startup_navigation_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/startup_navigation_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "STARTUP NAVIGATION TEST PASSED"
	exit 0
else
	echo "STARTUP NAVIGATION TEST FAILED (see above)" >&2
	exit 1
fi
