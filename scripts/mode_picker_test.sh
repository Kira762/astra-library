#!/bin/sh
# Runtime test for the Mode Picker element: mode-window enforcement,
# snap/set/callback order, dot rebuilds, knob stop positions, reset
# semantics and the add/remove bounds.
#
...
OUT="$TMPDIR_LOCAL/astra_mode_picker_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/mode_picker_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "MODE PICKER TEST PASSED"
	exit 0
else
	echo "MODE PICKER TEST FAILED (see above)" >&2
	exit 1
fi
