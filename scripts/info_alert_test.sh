#!/bin/sh
# Runtime test for the circle-alert info description on functional elements.
#
# The badge shipped with two defects this suite exists to pin down: the row's
# list layout was left on its default Name sort, so the badge landed *before*
# the element's name, and the floating description assigned a locale token
# (a table) straight to the label's Text, which the engine rejects - every tap
# on the badge died before the description could ever appear.
#
