...
OUT="$TMPDIR_LOCAL/astra_capsule_visibility_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/capsule_visibility_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "CAPSULE VISIBILITY TEST PASSED"
	exit 0
else
	echo "CAPSULE VISIBILITY TEST FAILED (see above)" >&2
	exit 1
fi
