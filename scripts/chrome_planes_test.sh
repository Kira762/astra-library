#!/bin/sh
# Runtime test for the window's three chrome planes: the topbar band, the rail
# band and the elements pane are flat painted surfaces that step darkest at the
# top, meet edge to edge, own the window-silhouette corners their edges touch,
# and fold away with the shell.
#
...
OUT="$TMPDIR_LOCAL/astra_chrome_planes_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/chrome_planes_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "CHROME PLANES TEST PASSED"
	exit 0
else
	echo "CHROME PLANES TEST FAILED (see above)" >&2
	exit 1
fi
