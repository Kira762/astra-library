#!/bin/sh
# Regression test for Anti Duplicate Window under rapid re-entry.
set -u
...
OUT="$TMPDIR_LOCAL/astra_anti_duplicate_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/anti_duplicate_window_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "ANTI DUPLICATE WINDOW TEST PASSED"
	exit 0
else
	echo "ANTI DUPLICATE WINDOW TEST FAILED (see above)" >&2
	exit 1
fi
