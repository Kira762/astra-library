#!/bin/sh
# Runtime test for functional-element lock metadata and cumulative mode locks.
set -u
...
OUT="$TMPDIR_LOCAL/astra_elements_lock_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/elements_lock_test.luau"
} > "$OUT"

if "$LUAU_BIN" "$OUT"; then
	echo "ELEMENT LOCK TEST PASSED"
	exit 0
else
	echo "ELEMENT LOCK TEST FAILED (see above)" >&2
	exit 1
fi
