#!/bin/sh
# Measure startup batching and test lazy panels against the standalone bundle.
set -u
...
OUT="$TMPDIR_LOCAL/astra_startup_$$.luau"
trap 'rm -f "$OUT"' EXIT

{
	...
	echo "end)()"
	echo ""
	cat "$ROOT/scripts/startup_test.luau"
} > "$OUT"
...
if "$LUAU_BIN" "$OUT"; then
	...
	echo "STARTUP TEST FAILED (see above)" >&2
	exit 1
fi
